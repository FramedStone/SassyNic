  ...
  document.addEventListener('DOMContentLoaded', async () => {
    ...
    filters_selection.forEach((filter) => {
      filter.addEventListener('change', () => {
        if (!filter.checked) {
          document
            .querySelector(`div.filters .${filter.id.replace('filter_', '')}`)
        ...
        if (filter.checked) {
          ...
          const children = document.querySelectorAll(
            'div.filters div.draggable-item:not([hidden]) div.draggable-item-child:not([hidden])'
          );
    ...
    function getAvailableSets() {
      ...
      let dataset_ = dataset.map((set) => {
        let filteredSet = set.filter(
          (course) => course.option.psc_disabled === '0' && course.option.status === 'Open'
        );
        let filteredSet_ = set.filter(
          (course) => course.option.psc_disabled === '1' || course.option.status === 'Closed'
        );
      ...
      dataset_ = dataset_.filter((set_, index) => dataset_[index].length === dataset[index].length);
    ...
    observeRanks('draggable-item-child', (changes) => {
      ...
      changes.forEach(({ element, newRank }) => {
        console.log(`new rank: ${newRank}, ${element.querySelector('label').textContent}`);
      });
    ...
    observeFiltersValues(
      ...
      table,
      dataset
    ); // pass dragndrop object to track newly created span(s), and fitness for real time updates
  });
})();
